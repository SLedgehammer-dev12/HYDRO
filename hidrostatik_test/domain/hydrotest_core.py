...
PRESSURE_VOLUME_LIMIT_FACTOR: Final[float] = 0.002
...
MICRO_TO_UNIT: Final[float] = 1e-6
...
@dataclass(frozen=True)
class PressureVolumeInputs:
    pipe: PipeModel
    a_micro_per_bar: float
    total_pressure_rise_bar: float
    k_factor: float
    actual_total_water_added_m3: float

    def __post_init__(self) -> None:
        if self.a_micro_per_bar <= 0:
            raise ValidationError("A degeri sifirdan buyuk olmalidir.")
        if self.total_pressure_rise_bar <= 0:
            raise ValidationError("Toplam basinclandirma basinci sifirdan buyuk olmalidir.")
        if self.k_factor <= 0:
            raise ValidationError("K faktoru sifirdan buyuk olmalidir.")
        if self.actual_total_water_added_m3 < 0:
            raise ValidationError("Ilave su hacmi negatif olamaz.")


@dataclass(frozen=True)
class PressureVolumeResult:
    theoretical_water_m3: float
    limit_m3: float
...
def evaluate_pressure_volume_limit(inputs: PressureVolumeInputs) -> PressureVolumeResult:
    deformation_term = inputs.pipe.elasticity_term + inputs.a_micro_per_bar
    if deformation_term <= 0:
        raise ValidationError("Basinc-hacim hesabi icin carpan pozitif olmalidir.")

    theoretical_water_m3 = (
        deformation_term
        * MICRO_TO_UNIT
        * inputs.pipe.internal_volume_m3
        * inputs.total_pressure_rise_bar
        * inputs.k_factor
    )
    if theoretical_water_m3 <= FLOAT_TOLERANCE:
        raise ValidationError("Teorik su hacmi hesaplanamadi.")

    limit_m3 = theoretical_water_m3 * (1.0 + PRESSURE_VOLUME_LIMIT_FACTOR)
    excess_ratio = (
        (inputs.actual_total_water_added_m3 - theoretical_water_m3) / theoretical_water_m3
        if theoretical_water_m3 > 0
        else float("inf")
    )
    within_limit = inputs.actual_total_water_added_m3 <= limit_m3 + FLOAT_TOLERANCE

    return PressureVolumeResult(
        theoretical_water_m3=theoretical_water_m3,
        limit_m3=limit_m3,
        actual_water_m3=inputs.actual_total_water_added_m3,
