...
class WaterPropertyValidationTests(unittest.TestCase):
    ...
    def test_pressure_volume_within_limit(self) -> None:
        from hidrostatik_test.domain.hydrotest_core import (
            PRESSURE_VOLUME_LIMIT_FACTOR,
            PipeSection,
            PressureVolumeInputs,
            evaluate_pressure_volume_limit,
        )
        pipe = PipeSection(outside_diameter_mm=219.1, wall_thickness_mm=8.18, length_m=1000.0)
        inputs = PressureVolumeInputs(
            pipe=pipe,
            a_micro_per_bar=47.193,
            total_pressure_rise_bar=80.0,
            k_factor=1.02,
            actual_total_water_added_m3=pipe.internal_volume_m3 * 1e-6 * (pipe.elasticity_term + 47.193) * 80.0 * 1.02 * 0.999,
        )
        result = evaluate_pressure_volume_limit(inputs)
        self.assertTrue(result.within_limit)
        self.assertLessEqual(result.excess_ratio, PRESSURE_VOLUME_LIMIT_FACTOR)

    def test_pressure_volume_exceeds_limit(self) -> None:
        from hidrostatik_test.domain.hydrotest_core import (
            PRESSURE_VOLUME_LIMIT_FACTOR,
            PipeSection,
            PressureVolumeInputs,
        ...
        inputs = PressureVolumeInputs(
            ...
            total_pressure_rise_bar=80.0,
            k_factor=1.02,
            actual_total_water_added_m3=pipe.internal_volume_m3 * 1e-6 * (pipe.elasticity_term + 47.193) * 80.0 * 1.02 * 1.005,
        )
        result = evaluate_pressure_volume_limit(inputs)
        self.assertFalse(result.within_limit)
        self.assertGreater(result.excess_ratio, PRESSURE_VOLUME_LIMIT_FACTOR)
