...
class UiWorkflowTests(unittest.TestCase):
    ...
    def test_input_panels_are_embedded_in_tabs(self) -> None:
        self.root.update_idletasks()
        self.app.notebook.select(0)
        self.app._on_tab_changed()
        self.root.update_idletasks()
        self.assertTrue(self.app.air_vars["temperature_c"].get() is not None)

        self.app.notebook.select(1)
        self.app._on_tab_changed()
        self.root.update_idletasks()
        self.assertTrue(self.app.pressure_vars["temperature_c"].get() is not None)

        self.app.notebook.select(2)
        self.app._on_tab_changed()
        self.root.update_idletasks()
        self.assertTrue(self.app.field_vars["pig_distance_m"].get() is not None)
    ...
    def test_apply_available_update_uses_selected_download_directory(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("hidrostatik_test.ui.app.messagebox.askyesnocancel", return_value=False), patch(
                "hidrostatik_test.ui.app.messagebox.askyesno", return_value=True
            ), patch(
